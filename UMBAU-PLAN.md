...
| # | Sub-Task | Ergebnis |
|---|---|---|
| **M0.1** | **Bestand sichern** — ✅ *abgeschlossen* | ✅ Upgrade-Diff als ein Block committet (`2723059`) · ✅ `game.sudoku` untrackt, `.gitignore` um Secrets/`*.slnx`/`.idea` ergänzt (`c320fdf`) · ✅ NRE in `Vibration.cs` an der Ursache behoben (`3c24494`) · ✅ Tag `prototype-v0` gesetzt und gepusht · ✅ **Kompilierprüfung bestanden** (Editor-Konsole 0 Errors; die 14 `missing script`-Warnings stammen vom entfernten ScreenLogger und wandern nicht mit).<br>❌ **Bildschirmaufnahme verworfen** — siehe §5.1. |
...
|---|---|---|---|
| **D-A** | **Unity-Version.** Installiert ist ausschließlich `6000.5.4f1` — das ist Unity 6.5, ein *Update Release*, kein LTS. Recherchiert (07/2026): aktueller LTS ist **Unity 6.3 LTS** (`6000.3.x`), Support bis **Dezember 2027**. Unity 6.0 LTS läuft im **Oktober 2026** aus und scheidet damit aus. | **Unity 6.3 LTS (`6000.3.x`, neuester Patch) im Hub installieren**, neues Projekt darauf anlegen. Update Releases sind laut Unity produktionsreif, haben aber keinen 2-Jahres-Support-Horizont — bei einem Vorhaben über mehrere Meilensteine ist der LTS-Zweig die billigere Wahl. Ein Versionswechsel nach M0 ist deutlich teurer. | vor M0.2 |
